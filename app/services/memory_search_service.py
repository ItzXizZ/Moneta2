...
class MemorySearchService:
                ...
                if api_results:
                    ...
                    for result in api_results:
                        if isinstance(result, dict):
                            ...
                            status = '[OK]' if relevance_score >= self.min_relevance else '[SKIP]'
                            print(f"   API result: '{result.get('memory', {}).get('content', 'N/A')[:30]}...' relevance: {relevance_score:.3f} {status}")
                    
                    if filtered_api_results:
                        print(f"   [INFO] Found {len(filtered_api_results)} STRICT filtered memories via API fallback (from {len(api_results)} total)")
                        return filtered_api_results[:self.max_injected]
                    else:
                        print(f"   [INFO] API returned {len(api_results)} memories but NONE met STRICT {self.min_relevance} relevance threshold")
        except Exception as e:
                print(f"   [WARN] API search fallback failed: {e}")
        
        return []
    
    def _log_search_results(self, results):
        """Log the search results for debugging"""
        print(f"[INFO] Found {len(results)} STRICT filtered memories (relevance >= {self.min_relevance}):")
        for i, result in enumerate(results):
            ...
            if result['relevance_score'] < self.min_relevance:
                print(f"       [ERROR] BUG: This memory passed strict filter but score {result['relevance_score']:.3f} < {self.min_relevance}!")
    ...
    def format_memories_for_injection(self, memory_context):
        ...
        print(f"[INFO] Injected {len(memory_context[:3])} memories into prompt")
